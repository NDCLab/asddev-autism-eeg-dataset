...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% This script is going to be edited by Kianosh Hosseini and Dr. Brittany
% Manning for the GibuBidu Data analysis.
% Data structure of this analysis should be kept the same on the HPC.
% The folder named "weill-nest-dataset" should be uploaded to home/data/NDClab/datasets/weill-nest-dataset/
% Please upload subfolders to their corresponding folders. They have the
% same name.
% After uploading data to the HPC, change the 'main_dir' below accordingly.
% 
%
%
% ************************************************************************
...
% if using OneDrive use this: main_dir = '/Users/khoss005/OneDrive - Florida International University/weill-nest-dataset';
main_dir = '/home/data/NDClab/datasets/weill-nest-dataset'; % Directory on the FIU HPC
% Comment line above and uncomment line below if you run it on your local
% computer. Make sure to change the directory accordingly.
%main_dir = '/Users/khoss005/OneDrive - Florida International University/weill-nest-dataset';
%% Setting up other things

%Location of MADE and ADJUSTED-ADJUST scripts
addpath(genpath([main_dir filesep 'code' filesep 'MADE-EEG-preprocessing-pipeline']));% enter the path of the EEGLAB folder in this line

%Location of "EEG
addpath(genpath([main_dir filesep 'code' filesep 'eeglab2021.1']));% enter the path of the EEGLAB folder in this line

%remove path to octave functions inside matlab to prevent errors when
rmpath([main_dir filesep 'code' filesep 'eeglab2021.1' filesep 'functions' filesep 'octavefunc' filesep 'signal'])

% 1. Enter the path of the folder that has the raw data to be analyzed
rawdata_folders = [main_dir filesep 'sourcedata' filesep 'GibuBidu_Data'];
cd (rawdata_folders) 
data_folder_lists = dir; 
data_folder_lists = data_folder_lists(~ismember({data_folder_lists.name},{'.', '..', '.DS_Store'}));
data_folder_lists = {data_folder_lists.name};
data_folder_lists = string(data_folder_lists);
%for lisar1=1:length(data_folder_lists) % This for loop lists the folders that include two or more subfolders containing the main data files
for lisar1=5 % This loop lists the folders under GibuBidu_Data
    rawdata_folders2 = fullfile(rawdata_folders, data_folder_lists(lisar1));
    cd (rawdata_folders2) 
    data_folder_lists2 = dir; 
    data_folder_lists2 = data_folder_lists2(~ismember({data_folder_lists2.name},{'.', '..', '.DS_Store'}));
    data_folder_lists2 = {data_folder_lists2.name};
    data_folder_lists2 = string(data_folder_lists2);
    %for lisar2=1:length(data_folder_lists2) % This for loop lists the folders containing the main data files
    for lisar2=1:length(data_folder_lists2) % This loop lists the folders under each cohort. Specify which subfloder you want to select.
        rawdata_location = fullfile(rawdata_folders2, data_folder_lists2(lisar2));
        rawdata_location = char(rawdata_location);


        % 2. Enter the path of the folder where you want to save the processed data
        output_location = [main_dir filesep 'derivatives' filesep 'GibuBidu_Processed'];
        output_location = fullfile(output_location, data_folder_lists(lisar1), data_folder_lists2(lisar2));
        output_location = char(output_location);
        %%%%%%%%%%%%
        % 3. Enter the path of the channel location file
        channel_locations = [main_dir filesep 'sourcedata' filesep 'Raw_Data_Info' filesep 'chanlocs.ced'];
        %%%%%%%%%%%%
        ...
        filter_timeoffset = 0;     % anti-aliasing time offset (in milliseconds). 0 = No time offset %%%For EGI system we need to check if it has been resolved in the version of the EEG system that we have used to collect data. If not, we should specify this parameter.
        stimulus_timeoffset   = 30.45; % stimulus related time offset (in milliseconds). 0 = No time offset
        ...
        task_event_markers = {'new-' 'old+' 'orig'}; % enter all the event/condition markers
        ...
        frontal_channels = {'E10' 'E5',}; % If you set interp_epoch = 1, enter the list of frontal channels to check (see manuscript for detail)
        ...
        datafile_names=dir([rawdata_location filesep '*.mff']);
        ...
        %     EEG = pop_loadset( 'filename', strrep(datafile_names{subject}, ext, '_ica_data_immediate.set'), 'filepath', [output_location filesep 'ica_data' filesep]);
            ...
            edit_event_markers_erpCore_ern
            ...
            %% STEP 13.5: DELETE First 4 Epochs
        ...
        writetable(report_table, [output_location filesep 'MADE_preprocessing_report_', datestr(now,'dd-mm-yyyy'),'.csv']);
    end
end